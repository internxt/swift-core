...
/// RIPE Message Digest 160 Swift implementation
public struct RIPEMD160 {
    ...
    /// Incrementally update the hash function with the given data.
    ...
    public mutating func finalize() -> Data {
        ...
        buffer = Data()
        let result = [MDbuffer.0, MDbuffer.1, MDbuffer.2, MDbuffer.3, MDbuffer.4]
        return result.withUnsafeBytes { Data($0) }
    }
    
    // MARK: - Internal Implementation
    /// Helper functions (originally macros in rmd160.h)
    /// - Parameter X: `UnsafePointer<UInt32>`
    private mutating func compress(_ X: UnsafePointer<UInt32>) {
        ...
        var (aa, bb, cc, dd, ee) = MDbuffer
        var (aaa, bbb, ccc, ddd, eee) = MDbuffer
