...
final class BIP39Tests: XCTestCase {
    ...
    private let crypto = CryptoUtils()

    /// (expected entropy as hex, mnemonic)
    ...
    func testMnemonicToEntropyVectors() throws {
        for (expected, mnemonic) in Self.vectors {
            let entropy = try crypto.mnemonicToEntropy(mnemonic)
            XCTAssertEqual(hex(entropy), expected, "entropy for: \(mnemonic)")
        }
    }

    /// A 24-word mnemonic, which is what Internxt issues, must give 32 bytes —
    /// the size the mail keystore derivation expects.
    func testInternxtSizedMnemonicYields32Bytes() throws {
        let mnemonic = Self.vectors.first { $0.1.split(separator: " ").count == 24 }!.1
        XCTAssertEqual(try crypto.mnemonicToEntropy(mnemonic).count, 32)
    }

    func testRejectsTamperedMnemonic() {
        // Swapping a word keeps it in the wordlist but breaks the checksum.
        var words = Self.vectors[0].1.split(separator: " ").map(String.init)
        words[0] = "zoo"
        XCTAssertThrowsError(try crypto.mnemonicToEntropy(words.joined(separator: " ")))
    }

    func testRejectsUnknownWord() {
        XCTAssertThrowsError(try crypto.mnemonicToEntropy("notaword " + Self.vectors[0].1))
    }
}
